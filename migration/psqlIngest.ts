...
const dump: Dump = JSON.parse(fs.readFileSync(process.argv[3], { encoding: 'utf8' }));
...
async function inject() {
    ...
    try {
        ...
        await insertType('player_hand', playerHandColumns, dump.playerHands.map(sanitizePlayerHand), 100);
    } finally {
        client.end();
    }
}
...
async function insertType<T>(table: string, columnMapping: [keyof T, string][], data: T[], chunkSize: number) {
...
async function insertChunk<T>(table: string, columnMapping: [keyof T, string][], chunk: T[]) {
...
async function updateSequenceToMax(table: string, column: string = 'id') {
