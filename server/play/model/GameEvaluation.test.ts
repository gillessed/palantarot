...
function expectGetEarningsWithDog(bid: number, dog: Card[], expectedPoints: number, expectedBouts: Card[]) {
  const biddingTeam = ["bidder"];
...
function newTrick(trick_num: number, players: PlayerId[], winner: PlayerId, ...cards: Card[]): CompletedTrick {
  return {
    trick_num,
    cards,
    players,
