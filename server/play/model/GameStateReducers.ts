...
import { Action, Bid, BidAction, BiddingCompletedTransition, BidValue, Call, CompletedBids, CompletedGameState, CompletedTrick, CompletedTrickTransition, CurrentBids, DealtHandTransition, DeclareSlam, DogRevealTransition, DummyPlayer, GameAbortedTransition, GameCompletedTransition, GameStartTransition, JokerExchangeState, Outcome, PlayerEvent, PlayerId, PlayersSetTransition, SetDogAction, ShowDogToObservers, ShowTrumpAction, ShowTrumpState } from "./GameEvents";
import { BiddingBoardState, BiddingStateActions, BiddingStates, BoardReducer, BoardState, CompletedBoardState, CompletedStateActions, DealtBoardState, DogRevealAndExchangeBoardState, DogRevealStateActions, DogRevealStates, GameplayState, NewGameActions, NewGameBoardState, NewGameStates, PartnerCallBoardState, PartnerCallStateActions, PartnerCallStates, PlayingBoardState, PlayingStateActions, PlayingStates, ReducerResult } from "./GameState";

export type GameReducerMap = { [state: string]: BoardReducer<any, any, any> }

export function buildGameStateReducer(): GameReducerMap {
  return {
    [GameplayState.NewGame]: newGameBoardReducer,
    [GameplayState.Bidding]: biddingBoardReducer,
    [GameplayState.PartnerCall]: partnerCallBoardReducer,
    [GameplayState.DogReveal]: dogRevealAndExchangeBoardReducer,
    [GameplayState.Playing]: playingBoardReducer,
    [GameplayState.Completed]: completedBoardReducer,
  };
}
...
function showTrumpActionReducer<T extends DealtBoardState>(state: T, action: ShowTrumpAction): ReducerResult<T> {
  ...
  if (state.shows.indexOf(action.player) >= 0) {
    ...
    throw GameErrors.invalidTrumpShow(action, getTrumps(state.hands[player_num]));
