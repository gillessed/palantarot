...
export class GameRecordService {
  ...
  public getMonthResults = async (req: Request, res: Response) => {
    ...
    const deltaCutoff = moment
      .tz(IMonth.westernTimezone)
      .startOf('day')
      .utc()
      .format('YYYY-MM-DD HH:mm:ss');

    try {
      const allResults = await this.getMonthResultsForRole(
        startDate,
        endDate,
        deltaCutoff
      );
      const bidderResults = await this.getMonthResultsForRole(
        startDate,
        endDate,
        deltaCutoff,
        Role.BIDDER
      );
      const partnerResults = await this.getMonthResultsForRole(
        startDate,
        endDate,
        deltaCutoff,
        Role.PARTNER
      );
      const oppositionResults = await this.getMonthResultsForRole(
        startDate,
        endDate,
        deltaCutoff,
        Role.OPPOSITION
  ...
  private getMonthResultsForRole = async (
    startDate: string,
    endDate: string,
    deltaCutoff: string,
    role?: Role
  ) => {
    const allResults = await this.gameDb.queryResultsBetweenDates(
      startDate,
      endDate,
      role
    );
    const deltaResults = await this.gameDb.queryResultsBetweenDates(
      deltaCutoff,
      endDate,
      role
    );
  ...
  private mapRoleResult = (
    resultMap: Map<string, Partial<Result>>,
    role: Role
  ) => {
  ...
  public getGamesInRange = (req: Request, res: Response) => {
    ...
    const startDate = moment
      .tz(body.startDateString, IMonth.westernTimezone)
      .format('YYYY-MM-DDThh:mm:ssZ');
    const endDate = moment
      .tz(body.endDateString, IMonth.westernTimezone)
      .format('YYYY-MM-DDThh:mm:ssZ');
    ...
    this.gameDb
      ...
      .catch((error: any) => {
        res.send({
          error: `Error loading games between ${startDate} and ${endDate}: ${error}`,
        });
      });
  };
  ...
  public getMonthGames = (req: Request, res: Response) => {
    ...
    this.gameDb
      ...
      .catch((error: any) => {
        res.send({
          error: `Error loading month games for ${startDate}: ${error}`,
        });
      });
  };
