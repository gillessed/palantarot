...
export class GameRecordQuerier {
  ...
  public queryGameWithId = (gameId: string): Promise<GameRecord> => {
    const sqlQuery = QueryBuilder.select("hand")
      .star()
      .join("player_hand", "INNER", QueryBuilder.compare().compareColumn("hand.id", "=", "player_hand.hand_fk_id"))
    ...
    if (query.player) {
      sqlQuery.join(
        QueryBuilder.subselect("player_hand", "p")
          .star()
          .where(QueryBuilder.compare().compareValue("player_fk_id", "=", query.player))
      ...
      sqlQuery.join(
        QueryBuilder.subselect("hand", "p").star().orderBy("timestamp", "desc").limit(query.count, query.offset),
        "INNER",
  ...
  public search = async (searchQuery: SearchQuery): Promise<GameRecord[]> => {
    ...
    for (const playerQuery of searchQuery.playerQueries) {
      ...
      sqlQuery.join(toJoin, "INNER", QueryBuilder.contains("hand.id", tableId + ".hand_fk_id"));
    }
  ...
  public saveGame = async (game: GameRecord): Promise<any> => {
    ...
    if (game.id) {
      ...
      upsertHandSqlQuery = QueryBuilder.update("hand").where(QueryBuilder.compare().compareValue("id", "=", game.id));
    } else {
      timestamp = moment().utc().format("YYYY-MM-DD HH:mm:ss");
      upsertHandSqlQuery = QueryBuilder.insert("hand").v("timestamp", timestamp).return("id");
    }
  ...
  public deleteGame = (gameId: string): Promise<any> => {
    ...
    return this.db.beginTransaction().then((transaction) => {
      return transaction
        .query(deleteHandSqlQuery.getIndexedQueryString(), deleteHandSqlQuery.getValues())
        .then(() => {
