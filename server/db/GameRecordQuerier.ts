...
export class GameRecordQuerier {
  ...
  public queryGameWithId = (gameId: string): Promise<GameRecord> => {
    const sqlQuery = QueryBuilder.select('hand')
      ...
      .where(QueryBuilder.compare().compareValue('hand.id', '=', gameId));
  ...
  public queryResultsBetweenDates = (
    startDate: string,
    endDate: string,
    role?: Role
  ): Promise<RoleResult[]> => {
    ...
    switch (role) {
      ...
      case Role.OPPOSITION:
        comparison
          .compareValue('was_bidder', '=', 'false')
          .compareValue('was_partner', '=', 'false');
        break;
    }
  ...
  public queryRecentGames = async (
    query: RecentGameQuery
  ): Promise<GameRecordPartial[] | GameRecord[]> => {
    ...
    if (query.player) {
      sqlQuery.join(
        QueryBuilder.subselect('player_hand', 'p')
          .star()
          .where(
            QueryBuilder.compare().compareValue(
              'player_fk_id',
              '=',
              query.player
            )
    ...
    const result = await this.db.query(
      sqlQuery.getIndexedQueryString(),
      sqlQuery.getValues()
    );
  ...
  public queryGamesBetweenDates = (
    ...
    if (playerId) {
      comparison.columnIn(
        'hand.id',
        QueryBuilder.select('player_hand').c('hand_fk_id')
      );
    }
    const sqlQuery = QueryBuilder.select('hand')
      .star()
  ...
  public search = async (searchQuery: SearchQuery): Promise<GameRecord[]> => {
    if (
      searchQuery.playerQueries.length < 1 &&
    ...
    for (const playerQuery of searchQuery.playerQueries) {
      ...
      const toJoin = QueryBuilder.subselect('player_hand', tableId)
        .star()
        .orderBy('timestamp', 'desc');
  ...
  public saveGame = async (game: GameRecord): Promise<any> => {
    ...
    if (game.id) {
      ...
      upsertHandSqlQuery = QueryBuilder.update('hand').where(
        QueryBuilder.compare().compareValue('id', '=', game.id)
      );
    } else {
    ...
    if (game.id) {
      ...
      const deleteOldHandsSqlQuery = QueryBuilder.delete('player_hand').where(
        QueryBuilder.compare().compareValue('hand_fk_id', '=', game.id)
      );
      await transaction.query(
        deleteOldHandsSqlQuery.getIndexedQueryString(),
        deleteOldHandsSqlQuery.getValues()
      );
    }
    const result = await transaction.query(
      upsertHandSqlQuery.getIndexedQueryString(),
      upsertHandSqlQuery.getValues()
    );
    let gameId: string;
    if (game.id) {
      gameId = game.id;
    ...
    insertPlayerHandsSqlQueries.push(
      this.createInsertHandQuery(
        handData.bidder,
        gameId,
        timestamp,
        true,
    ...
    handData.opposition.forEach(hand => {
      insertPlayerHandsSqlQueries.push(
        this.createInsertHandQuery(hand, gameId, timestamp, false, false)
      );
    });
    await Promise.all(
      insertPlayerHandsSqlQueries.map((sqlQuery): Promise<any> => {
  ...
  public deleteGame = (gameId: string): Promise<any> => {
    const deleteHandSqlQuery = QueryBuilder.delete('hand').where(
      QueryBuilder.compare().compareValue('id', '=', gameId)
    );
    const deletePlayerHandsSqlQuery = QueryBuilder.delete('player_hand').where(
    ...
    return this.db.beginTransaction().then(transaction => {
      return transaction
        .query(
          deleteHandSqlQuery.getIndexedQueryString(),
          deleteHandSqlQuery.getValues()
        )
        .then(() => {
          return transaction.query(
            deletePlayerHandsSqlQuery.getIndexedQueryString(),
            deletePlayerHandsSqlQuery.getValues()
          );
        })
        .then(() => {
          return transaction.commit();
  ...
  public getSlamGames = (): Promise<GameRecord[]> => {
    const sqlQuery = QueryBuilder.select('hand')
      ...
      .where(QueryBuilder.compare().compareValue('hand.points', '>=', 260))
  ...
  private getPlayerHands(hands: any[]): HandData {
    const bidderData = this.getHandData(
      hands.find((hand: any) => hand['was_bidder'])
    );
