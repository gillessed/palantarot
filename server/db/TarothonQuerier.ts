...
import { type Tarothon, type NewTarothon } from "../model/Tarothon.ts";
...
export class TarothonQuerier {
  ...
  public getTarothon = (id: string): Promise<Tarothon> => {
    const sqlQuery = QueryBuilder.select("tarothon").star().where(QueryBuilder.compare().compareValue("id", "=", id));
  ...
  public insertTarothon = (data: NewTarothon): Promise<string> => {
    const sqlQuery = QueryBuilder.insert("tarothon").v("begin_date", data.begin).v("end_date", data.end).return("id");
    return this.db.query(sqlQuery.getIndexedQueryString(), sqlQuery.getValues()).then((result: QueryResult) => {
  ...
  public deleteTarothon = (id: string): Promise<any> => {
    const sqlQuery = QueryBuilder.delete("tarothon").where(QueryBuilder.compare().compareValue("id", "=", id));
    return this.db.query(sqlQuery.getIndexedQueryString(), sqlQuery.getValues());
  };
