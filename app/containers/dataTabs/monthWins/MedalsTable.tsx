...
export class MedalsTable extends React.PureComponent<Props, {}> {
  public render() {
    const medalRecords = this.countMonthMedals(this.props.groupedMonthlyScores).sort(
      integerComparator(
    ...
    return (
      <div className="sub-container">
        ...
        <HTMLTable className="slam-count-table" bordered>
          ...
          <tbody>{medalRecords.map((medalRecord: MedalRecord) => this.renderMedalRecord(medalRecord))}</tbody>
        </HTMLTable>
      </div>
    );
  }
  ...
  private countMonthMedals(groupedMonthlyScores: MonthlyScore[][]): MedalRecord[] {
    ...
    return count<MedalEntry>(medals, (medal: MedalEntry) => medal.id, [
      firstsAggregator,
      secondsAggregator,
      thirdsAggregator,
