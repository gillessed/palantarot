...
const recentGamesService = generatePropertyService<
  RecentGameQuery,
  GameRecord[]
>('recentGames', (api: ServerApi) => {
...
export const recentGamesLoader: Loader<
  ReduxState,
  RecentGameQuery,
  GameRecord[]
> = {
  get: (state: ReduxState, _: RecentGameQuery) => state.recentGames,
  load: (dispatchers: Dispatchers, query: RecentGameQuery, force?: boolean) =>
    dispatchers.recentGames.request(query, force),
};
