...
export interface ServiceActions<REQUEST, RESULT> {
  ...
  success: TypedAction.Definition<
    string,
    {arg: REQUEST[]; result: Map<REQUEST, RESULT>}
  >;
...
export function generateServiceActions<REQUEST, RESULT>(
  prefix: string
): ServiceActions<REQUEST, RESULT> {
  ...
  return {
    ...
    success: TypedAction.define(name('success'))<{
      arg: REQUEST[];
      result: Map<REQUEST, RESULT>;
    }>(),
...
export function generatePropertyActions<REQUEST, RESULT>(
  prefix: string
): PropertyActions<REQUEST, RESULT> {
  const name = actionName(prefix);
  return {
    request: TypedAction.define(name('request'))<REQUEST>(),
