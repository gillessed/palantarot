import { type Loadable, LoadableCache } from "./loadable";
import { TypedReducer } from "redoodle";
import { type ServiceActions, type PropertyActions } from "./serviceActions";
...
export function generateServiceReducer<ARG, RESULT, KEY = ARG>(
  ...
  reducerBuilder = TypedReducer.builder<LoadableCache<KEY, RESULT>>().withDefaultHandler(
    (state = LoadableCache.create<KEY, RESULT>()) => state
  )
) {
  function loadingReducer(cache: LoadableCache<KEY, RESULT>, args: ARG[]) {
    return cache.keysLoading(...args.map(argMapper));
  }
  function successReducer(cache: LoadableCache<KEY, RESULT>, result: { arg: ARG[]; result: Map<ARG, RESULT> }) {
  ...
  function errorReducer(cache: LoadableCache<KEY, RESULT>, error: { arg: ARG[]; error: Error }) {
...
export function generatePropertyReducer<ARG, RESULT>(
  ...
  reducerBuilder = TypedReducer.builder<Loadable<ARG, RESULT>>().withDefaultHandler(
    (state = { key: undefined, loading: false }) => state
  )
) {
  function loadingReducer(state: Loadable<ARG, RESULT>) {
    return { ...state, loading: true };
  }
  function successReducer(_: Loadable<ARG, RESULT>, result: { arg: ARG; result: RESULT }) {
