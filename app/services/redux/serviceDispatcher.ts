  ...
  return class implements ServiceDispatcher<ARG> {
    ...
    public request(args: ARG[], force?: boolean) {
      ...
      if (uncachedArgs.length >= 1) {
        this.debounceTime
          ? this.requestDebounced(uncachedArgs)
          : this.requestInternal(uncachedArgs);
      }
    }

    private requestDebounced = debounce(
...
export function generatePropertyDispatcher<ARG, RESULT>(
  actionCreators: PropertyActions<ARG, RESULT>
) {
  return class implements PropertyDispatcher<ARG> {
    protected debounceTime = 0;
    ...
    public request(arg: ARG, force?: boolean) {
      ...
      this.debounceTime
        ? this.requestDebounced(arg)
        : this.requestInternal(arg);
    }
...
export type CacheFunction<KEY, RESULT> = (
  arg: KEY,
  loadable?: Loadable<KEY, RESULT>
) => boolean;
...
export function loading<KEY, RESULT>(): CacheFunction<KEY, RESULT> {
  return (_: KEY, loadable?: Loadable<KEY, RESULT>) =>
    loadable ? loadable.loading : false;
}
