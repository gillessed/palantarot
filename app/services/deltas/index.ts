...
const deltasService = generatePropertyService<DeltasRequest, Deltas>(
  'deltas',
  (api: ServerApi) => {
...
export const deltasLoader: Loader<ReduxState, DeltasRequest, Deltas> = {
  ...
  load: (dispatchers: Dispatchers, request: DeltasRequest, force?: boolean) =>
    dispatchers.deltas.request(request, force),
};
